...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!pfxFile || !password) {
      return NextResponse.json({ error: 'Falta el archivo del certificado o la contraseña' }, { status: 400 });
    }
    ...
    try {
      ...
      if (!certBagArray || !pkeyBagArray || certBagArray.length === 0 || pkeyBagArray.length === 0) {
      ...
      // Generar un token para representar la sesión del certificado (simplificado para este ejemplo)
      ...
      await fs.unlink(tempFilePath);
      return NextResponse.json({ error: 'Contraseña incorrecta o certificado inválido' }, { status: 400 });
    }
    
  } catch (error) {
    console.error('Error al procesar el certificado:', error);
    return NextResponse.json({ error: 'Error al procesar el certificado' }, { status: 500 });
  }
}